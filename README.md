...
The blank-page problem, solved: the first draft of every contract can be generated from what the app already knows — the model's columns, and the params calls already sitting in the controller, in either spelling (`params.require(...).permit(...)` or Rails 8's `params.expect(...)`).
...
```sh
bundle install
bundle exec rspec      # 210 examples
bundle exec rubocop
```
