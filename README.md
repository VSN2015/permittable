...
Rails merges its own keys into `params`: `controller`, `action`, and `format` from the router, plus `authenticity_token`, `_method`, `utf8`, and `commit` from an ordinary form POST. All seven are exempt at the top level. So are two that depend on the request: the route's **path parameters** (`PATCH /users/1` merges `id`, which the exported OpenAPI documents as a path parameter rather than a body field), and **ParamsWrapper's copy of a JSON body** under the controller's wrapper key (`user` for `UsersController`) — but only when Rails made that copy; a client that sends `user` itself is flagged like any other extra key. Either way `unknown: :error` flags what the *client* got wrong rather than what the framework added, and a contract that *declares* a field of one of these names has it validated as usual. Inside a `root:` or a nested hash there is no such exemption, because nothing legitimately injects keys there — and a standalone `Contract` exempts nothing at all, having neither a router, a form, nor a request.

The exemption covers the *check* only. Monitor mode still hands back the form keys, the path parameters and the wrapper's copy in its raw pass-through, where behaving exactly like the pre-contract app is the whole promise and a legacy action may read `params[:id]` or `_method` itself; only the router's three are dropped there.
