...
- **One contract, one envelope.** A contract has one `root:`, so the generator picks it from every call in the file before it drafts any field. Each envelope is a candidate (every envelope of an `expect(post: [...], comment: [...])` call counts), and so are the rootless calls taken together. The winner is the candidate with the most **parsed** fields (`*PERMITTED` counts for nothing). On a tie an envelope beats the rootless calls, then the first in the source wins. Only the winner's calls become fields. Everything else stays visible as a TODO that says why: `belongs to another envelope (search): search: [:q]` for a losing envelope, and `route or query param, not a body field: :id` for a rootless key once an envelope wins. That covers a Rails 8 scaffold's `Post.find(params.expect(:id))`, an index action's `params.permit(:page)`, and a route param in the same call (`params.expect(:id, user: [:name])`). A file with only rootless calls drafts a rootless contract.
- **A scan with no parsable fields falls back to the columns.** If every call is `permit(*PERMITTED)`, a contract with only TODO lines would raise `a contract must declare at least one field` when pasted. So the columns are drafted instead, with the scan's TODOs underneath. The scan's rootness is kept: a rootless controller gets a rootless draft, with the columns at the top level. With no columns either, there is no draft.
- **A key permitted in two shapes is drafted once.** A contract rejects a field declared twice. A nested hash and an array of hashes merge into the array of hashes, keeping the sub-keys of both. An array of scalars (`tags: []`) and a hash shape (`tags: [:a]`) accept different input, so neither is drafted, and the TODO asks you to declare the shape the actions share. Otherwise the richer shape wins over a scalar. Every conflict is named in a TODO (`tags is permitted as both a scalar and an array — drafted as the array`).
