...
- **One contract, one envelope.** A contract has one `root:`, so the generator picks it from every call in the file before it drafts any field. When the model is known, its own envelope (`post` for `Post`) wins outright if any call uses it, even `require(:post).permit(*PERMITTED)`. Otherwise each envelope is a candidate (every envelope of an `expect(post: [...], comment: [...])` call counts, and so does `expect(post: PERMITTED_PARAMS)`), and so are the rootless calls taken together. The winner is the candidate with the most **parsed** fields (`*PERMITTED` counts for nothing). On a tie an envelope beats the rootless calls, then the first in the source wins. A single-key lookup of `:id` or a `*_id` key, like a Rails 8 scaffold's `Post.find(params.expect(:id))`, is a route param: it never scores and is never drafted as a field. Only the winner's calls become fields. Everything else stays visible as a TODO that says why: `belongs to another envelope (search): search: [:q]` for a losing envelope, `route or query param, not a body field: :page` for a bare rootless key once an envelope wins, and `outside the post envelope, so not in this contract: tag_names: []` for an array or hash next to the envelope. A file with only rootless calls drafts a rootless contract.
- **A draft always declares a field, or there is no draft.** If no scanned line would declare one — every call is `permit(*PERMITTED)`, or the only scanned key is a `binary` column, which has no contract type — a contract of only TODO lines would raise `a contract must declare at least one field` when pasted. So the columns are drafted instead, with the scan's TODOs underneath. The scan's rootness is kept: a rootless controller gets a rootless draft, with the columns at the top level. If the columns cannot declare a field either, there is no draft.
